package com.rslakra.accountservice.service;

/**
 * @author Rohtash Lakra
 * @created 12/5/22 20:10 PM
 */
public class UserUtils {
//
// /**
//     * Returns the auth tracer of the auth.
//     *
//     * @param httpRequest
//     * @return
//     */
// public static String getRequestTracer(final GeminiHttpRequest httpRequest) {
//        return httpRequest.getHeader(JwtUtils.REQUEST_TRACER);
// }
//
// /**
//     * Returns the auth tracer of the auth.
//     *
//     * @param httpResponse
//     * @return
//     */
// public static String getRequestTracer(final CloseableHttpResponse httpResponse) {
//        if (httpResponse != null) {
// final Header header = httpResponse.getFirstHeader(JwtUtils.REQUEST_TRACER);
// if (header != null) {
//                return header.getValue();
// }
//        }
//
//        return null;
// }
//
// /**
//     * Returns the auth tracer of the auth.
//     *
//     * @param httpRequestBase
//     * @return
//     */
// public static String getRequestTracer(final HttpRequestBase httpRequestBase) {
//        if (httpRequestBase != null) {
// final Header header = httpRequestBase.getFirstHeader(JwtUtils.REQUEST_TRACER);
// if (header != null) {
//                return header.getValue();
// }
//        }
//
//        return null;
// }
}
