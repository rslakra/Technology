...
import com.rslakra.ratelimiter.fixedwindowcounter.controller.ShapeController;
...
class ShapeControllerRateLimiterTest {
    ...
    void setUp() {
        ...
        ShapeResponse mockResponse = new ShapeResponse();
        ...
        when(shapeService.calculate(any(ShapeRequest.class))).thenReturn(mockResponse);
        
        // Mock rate limiter to allow requests (will be overridden in specific tests)
    ...
    void testRequestsWithinRateLimit_ShouldSucceed() throws Exception {
        ...
        for (int i = 0; i < 50; i++) {
            mockMvc.perform(post("/api/v1/shapes/rectangle")
    ...
    void testRequestsExceedingRateLimit_ShouldReturn429() throws Exception {
        ...
        // Mock rate limiter to allow first 50 requests, then reject
        when(rateLimiter.tryConsume())
            .thenAnswer((Answer<Boolean>) invocation -> {
                int count = callCount.incrementAndGet();
                return count <= 50; // First 50 return true, 51st+ return false
            });
