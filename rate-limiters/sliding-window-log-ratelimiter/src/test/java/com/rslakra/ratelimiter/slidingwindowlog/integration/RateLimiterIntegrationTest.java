package com.rslakra.ratelimiter.slidingwindowlog.integration;
...
class RateLimiterIntegrationTest {
     ...
     * Note: Sliding window log maintains state as a singleton, so exact counts
     * may vary based on previous test runs and window state.
     */
    @Test
    ...
    void testConcurrentRequests_WithinRateLimit() throws Exception {
        ...
        // Sliding window log maintains state, so some requests may be rate limited
        // Allow for some rate limiting due to window state from previous tests
        assertTrue(successCount >= 5, 
            String.format("Expected at least 5 successful requests out of 10, but got %d. " +
                "Sliding window log may rate limit earlier due to window state.", successCount));
    }
