...
A Spring Boot application demonstrating rate limiting using the **Token Bucket Algorithm** implemented via Bucket4j library.

## Rate Limiting Pattern

This service implements the **Token Bucket Algorithm**, which is a rate limiting technique that allows bursts of traffic while maintaining an average rate limit over time.

### How Token Bucket Works

1. **Bucket Capacity**: The bucket holds a maximum number of tokens (e.g., 50 tokens)
2. **Token Refill**: Tokens are refilled at a specified rate (e.g., 50 tokens per minute)
3. **Request Processing**: Each request consumes one token from the bucket
4. **Rate Limiting**: If the bucket is empty, requests are rejected with HTTP 429 (Too Many Requests)
5. **Burst Allowance**: Allows bursts up to bucket capacity, then refills at constant rate

### Current Implementation

- **Algorithm**: Token Bucket with Intervally Refill Strategy
- **Rate Limit**: Configurable via `application.properties` (default: 50 requests per minute)
- **Refill Strategy**: Intervally (tokens are refilled evenly over the time window)
- **Library**: Bucket4j 7.6.0
...
- Industry-standard algorithm (widely used)

**Cons:**
- Requires external library (Bucket4j) or custom implementation
- Can allow bursts that exceed average rate (if bucket is full)
...
- **Rate Limit**: Configurable via `application.properties` (default: 50 requests per minute)
- **Refill Strategy**: Intervally (tokens refill evenly over the time window)
...
**Note**: The rate limit is applied per controller instance. In a distributed environment with multiple instances, each instance maintains its own rate limit bucket. Rate limit values can be customized via configuration properties (see [Configuration](#configuration) section).
...
```properties
# Rate Limiter Configuration
rate-limiter.capacity=50          # Maximum bucket capacity (number of tokens)
rate-limiter.refill-amount=50      # Number of tokens to refill per period
rate-limiter.refill-period=1m      # Refill period (supports: 1m, 60s, 1h, PT1M, etc.)
```
...
2. **Bucket4jRateLimiterTest** (Unit Tests)
...
3. **RateLimiterIntegrationTest** (`@SpringBootTest`)
...
- ✅ Token consumption and bucket state management
- ✅ Burst consumption up to capacity
- ✅ Token refill after refill period
...
### 7. **Multiple Algorithm Support**
   - **Current**: Token Bucket only (via Bucket4j)
   - **Improvement**: Support multiple algorithms (Token Bucket, Sliding Window Log, Fixed Window Counter, Sliding Window Counter, Leaky Bucket)
