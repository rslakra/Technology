package com.rslakra.ratelimiter.tokenbucket.integration;
...
class RateLimiterIntegrationTest {
     ...
     * Note: Token bucket maintains state as a singleton, so exact counts
     * may vary based on previous test runs and token refill timing.
     */
    @Test
    void testRateLimiting_First50RequestsSucceed_51stFails() throws Exception {
        ...
        // and potentially some rate-limited requests (due to token bucket state from previous tests)
    ...
    void testRateLimiting_IsPerEndpoint() throws Exception {
        ...
        // Rectangle endpoint may be rate limited (depending on token bucket state)
        int status = mockMvc.perform(post(RECTANGLE_ENDPOINT)
                .contentType(MediaType.APPLICATION_JSON)
                .content(VALID_REQUEST_BODY))
                .andReturn()
    ...
    void testConcurrentRequests_WithinRateLimit() throws Exception {
        ...
        // Token bucket maintains state, so some requests may be rate limited
        // Allow for some rate limiting due to bucket state from previous tests
        assertTrue(successCount >= 5, 
            String.format("Expected at least 5 successful requests out of 10, but got %d. " +
                "Token bucket may rate limit earlier due to bucket state.", successCount));
    }
